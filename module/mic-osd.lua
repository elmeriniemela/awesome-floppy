...
mic_osd_slider:connect_signal(
    'property::value',
    function()
        local mic_level = mic_osd_slider:get_value()
        awful.spawn('amixer -D pulse sset Capture ' .. mic_level .. '%', false)

        -- Update textbox widget text
        osd_value.text = mic_level .. '%'

        -- Update the mic slider if values here change
        awesome.emit_signal('widget::mic:update', mic_level)
...
-- The emit will come from the mic-slider
awesome.connect_signal(
    'module::mic_osd',
    function(mic)
        mic_osd_slider:set_value(mic)
    end
)
...
local mic_slider_osd = wibox.widget {
    icon,
    slider_osd,
    spacing = dpi(24),
